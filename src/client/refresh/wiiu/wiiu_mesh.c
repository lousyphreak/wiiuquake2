...
DrawAliasFrameLerp(dmdl_t *paliashdr, entity_t* entity, vec3_t shadelight)
{
	...
	if (colorOnly)
	{
		WiiU_UseProgram(&gl3state.si3DaliasColor);
	}
	else
	{
		WiiU_UseProgram(&gl3state.si3Dalias);
	}
	...
	YQ2_STATIC_ASSERT(sizeof(gl3_alias_vtx_t) == 9*sizeof(float), "invalid gl3_alias_vtx_t size");
	...
	while (1)
	{
		...
		if (count < 0)
		{
			...
			type = GX2_PRIMITIVE_MODE_TRIANGLE_FAN;
		}
		else
		{
		...
		if(type == GX2_PRIMITIVE_MODE_TRIANGLE_FAN)
		{
			uint16_t i;
			for(i=1; i < count-1; ++i)
			{
...
void
WiiU_DrawAliasModel(entity_t *entity)
{
	int i;
	dmdl_t *paliashdr;
	float an;
	...
	{
		WiiU_LightPoint(entity, entity->origin, shadelight);
	...
	if (entity->flags & RF_DEPTHHACK)
	{
		...
		WiiU_SetDepthRange(gl3depthmin, gl3depthmin + 0.3 * (gl3depthmax - gl3depthmin));
	}

	if (entity->flags & RF_WEAPONMODEL)
	{
		extern hmm_mat4 WiiU_MYgluPerspective(float fovy, float aspect, float zNear, float zFar);
		...
		if (r_gunfov->value < 0)
		{
			projMat = WiiU_MYgluPerspective(gl3_newrefdef.fov_y, screenaspect, 4, dist);
		}
		else
		{
			projMat = WiiU_MYgluPerspective(r_gunfov->value, screenaspect, 4, dist);
		}
		...
		if(gl_lefthand->value == 1.0F)
		{
			...
			//WiiU_UpdateUBO3D(); Note: WiiU_RotateForEntity() will call this,no need to do it twice before drawing
