 ...
 */
void
WiiU_EmitWaterPolys(msurface_t *fa)
{
	glpoly_t *bp;
	float scroll = 0.0f;
	...
	if (fa->texinfo->flags & SURF_FLOWING)
	{
		...
		if (scroll == 0.0f) // this is done in WiiU_DrawGLFlowingPoly() TODO: keep?
		{
			scroll = -64.0f;
		}
	}
	...
	WiiU_UseProgram(&gl3state.si3Dturb);

	for (bp = fa->polys; bp != NULL; bp = bp->next)
	{
		WiiU_BufferAndDraw3D(bp->vertices, bp->numverts, GX2_PRIMITIVE_MODE_TRIANGLE_FAN);
	}
}
...
void
WiiU_SetSky(char *name, float rotate, vec3_t axis)
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		image = (gl3image_t *)GetSkyImage(skyname, suf[i],
			r_palettedtexture->value, (findimage_t)WiiU_FindImage);
...
void
WiiU_AddSkySurface(msurface_t *fa)
{
	int i;
...
WiiU_DrawSkyBox(void)
{
	...
	WiiU_UseProgram(&gl3state.si3Dsky);
	...
	for (i = 0; i < 6; i++)
	{
		...
		WiiU_Bind(&sky_images[skytexorder[i]]->texture);
		...
		WiiU_BufferAndDraw3D(skyVertices, 4, GX2_PRIMITIVE_MODE_TRIANGLE_FAN);
	}
