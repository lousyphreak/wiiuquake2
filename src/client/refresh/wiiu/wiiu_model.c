...
const byte*
WiiU_Mod_ClusterPVS(int cluster, const gl3model_t *model)
{
...
extern void
WiiU_SubdivideSurface(msurface_t *fa, gl3model_t* loadmodel);
...
static int calcTexinfoAndFacesSize(byte *mod_base, const lump_t *fl, const lump_t *tl)
{
	...
	for (int surfnum = 0; surfnum < face_count; surfnum++, face_in++)
	{
		...
		if (texFlags & SURF_WARP)
		{
			...
			// WiiU_SubdivideSurface(out, loadmodel); /* cut up polygon for warps */
		...
		{
			// WiiU_LM_BuildPolygonFromSurface(out);
...
Mod_LoadFaces(gl3model_t *loadmodel, byte *mod_base, lump_t *l)
{
	...
	WiiU_LM_BeginBuildingLightmaps(loadmodel);
	...
	for (surfnum = 0; surfnum < count; surfnum++, in++, out++)
	{
		...
		{
			WiiU_LM_CreateSurfaceLightmap(out);
		}

		if (!(out->texinfo->flags & SURF_WARP))
		{
			WiiU_LM_BuildPolygonFromSurface(loadmodel, out);
		}
	}

	WiiU_LM_EndBuildingLightmaps();
}
...
Mod_LoadBrushModel(gl3model_t *mod, void *buffer, int modfilelen)
{
	...
	Mod_LoadTexinfo (mod->name, &mod->texinfo, &mod->numtexinfo,
		mod_base, &header->lumps[LUMP_TEXINFO], (findimage_t)WiiU_FindImage,
		gl3_notexture, 0);
		...
		case IDALIASHEADER:
			{
				mod->extradata = Mod_LoadMD2(mod->name, buf, modfilelen,
					mod->mins, mod->maxs,
					(struct image_s **)mod->skins, (findimage_t)WiiU_FindImage,
					&(mod->type));
				if (!mod->extradata)
				{
 ...
 */
void
WiiU_BeginRegistration(char *model)
{
	...
	if (mod)
	{
		...
		else
		{
			...
			mod->numframes = Mod_ReLoadSkins((struct image_s **)mod->skins,
				(findimage_t)WiiU_FindImage, mod->extradata, mod->type);
		}
	}
...
void
WiiU_EndRegistration(void)
{
	int i;
	gl3model_t *mod;

	if (Mod_HasFreeSpace() && WiiU_ImageHasFreeSpace())
	{
