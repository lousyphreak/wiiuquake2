...
WiiU_PushDlights(void)
{
	...
	for (i = 0; i < gl3_newrefdef.num_dlights; i++, l++)
	{
		...
		R_MarkLights(l, 1 << i, gl3_worldmodel->nodes, r_dlightframecount, WiiU_MarkSurfaceLights);
...
void
WiiU_LightPoint(entity_t *currententity, vec3_t p, vec3_t color)
{
	vec3_t end;
	float r;
	int lnum;
 ...
 */
void
WiiU_BuildLightMap(msurface_t *surf, int offsetInLMbuf, int stride)
{
	int smax, tmax;
	int r, g, b, a, max;
	...
	{
		ri.Sys_Error(ERR_DROP, "WiiU_BuildLightMap called for non-lit surface");
	}
